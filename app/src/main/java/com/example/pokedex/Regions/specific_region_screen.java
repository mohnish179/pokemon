package com.example.pokedex.Regions;
...
import com.example.pokedex.PokeApiService.pokeapiservice_get_generation_pokemon;
...
public class specific_region_screen extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       Intent intent=getIntent();
        recieve_region=intent.getIntExtra("region_sent",0);
        recieve_region++;
    ...
    private void get_info(int offset) {
        pokeapiservice_get_generation_pokemon service = retrofit.create(pokeapiservice_get_generation_pokemon.class);
        ...
        pokemonAnsCall.enqueue(new Callback<Pokemon_get_pokemon_for_generation>() {
            ...
            public void onResponse(Call<Pokemon_get_pokemon_for_generation> call, Response<Pokemon_get_pokemon_for_generation> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    Pokemon_get_pokemon_for_generation pokemonAns = response.body();
                    length=pokemonAns.getPokemon_species().size();

                    ArrayList<Pokemon> list_Pokemon = new ArrayList<>();
                    for(int i=0;i<length;i++) {

                        list_Pokemon.add(pokemonAns.getPokemon_species().get(i));
            ...
            @Override
            public void onFailure(Call<Pokemon_get_pokemon_for_generation> call, Throwable t) {
                a1 = true;
